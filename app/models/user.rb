class User < ActiveRecord::Base
  ...
  scope :order_by_last_period, -> {
    joins(:periods).having("COALESCE(periods.end_date, '2050-01-01') = COALESCE(MAX(periods.end_date), '2050-01-01')").group('periods.id').order('periods.end_date DESC, periods.id DESC')
  }
